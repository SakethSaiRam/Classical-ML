''' Two Layered Neural Network (Mini Batch Gradient Descent) '''
...
class NeuralNetwork:
    ...
    def __init__(self,lr=0.5,lamda=0.15,num_iters=50,hidden_layer_size=40,batch_size=500,activation='sigmoid'):
        ''' '__init__' takes arguments as Learning Rate(lr), Regularization Constant(lamda), Number of Iterations(num_iters), s1(Hidden Layer Size), Batch Size, func(Type of activation function) '''
    ...
    def fit(self,X,y,num_class):
        ...
        for i in range(self.num_iters):
            # Applying FP and BP in mini batches
            for j in range(m // self.bs):
                X_mini = X[j*self.bs:(j+1)*self.bs,:]
                y_mini = y_class[j*self.bs:(j+1)*self.bs,:]
                ...
                # Cost Function
                part1 = (-1 / self.bs) * np.sum(y_mini * np.log(np.abs(a3 + 1e-10)) + (1 - y_mini) * np.log(1 - a3 + 1e-10))
                part2 = (self.lamda / (2 * self.bs)) * (np.sum(self.theta1[:,1:] ** 2) + np.sum(self.theta2[:,1:] ** 2))
                J = part1 + part2
                it += 1
                ...
                theta2_temp[:, 0] = 0
                D2 = (1 / self.bs) * (delta2 + self.lamda * theta2_temp)
                D1 = (1 / self.bs) * (delta1 + self.lamda * theta1_temp)
